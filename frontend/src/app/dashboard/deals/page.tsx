...
export default function DealsPage() {
  ...
  const load = () => {
    api.get<Deal[]>('/deals').then(setDeals).catch(console.error);
  };
  ...
  const sendQuote = async (id: string) => {
    const res = await api.post<{ message: string }>(`/deals/${id}/quote`);
    alert(res.message);
    load();
  };
  ...
  return (
    <div className="page-wrap">
      <PageHeader
        ...
        subtitle="حوّل محادثات واتساب إلى صفقات وإيراد"
        actions={
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)}>
        <form onSubmit={create} className="modal-panel surface-card p-4 sm:p-6 space-y-3" style={{ width: 'min(100%, 28rem)' }}>
          ...
          <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
            placeholder="عنوان الصفقة" className="input-field" />
          <input type="number" value={form.value} onChange={(e) => setForm({ ...form, value: Number(e.target.value) })}
            placeholder="القيمة" className="input-field" />
          <input value={form.itemName} onChange={(e) => setForm({ ...form, itemName: e.target.value })}
            placeholder="اسم المنتج/الخدمة (اختياري)" className="input-field" />
          ...
          <div key={d._id} className="mode-card">
            <div className="flex items-start justify-between gap-2 mb-3">
              <div>
                ...
                <p className="text-xs text-[var(--muted)]">{d.customerId?.name} · {d.customerId?.phone}</p>
              </div>
              <p className="font-bold text-[var(--teal)]">{d.value.toLocaleString('ar')} {d.currency}</p>
            </div>
            <select
              value={d.stage}
              onChange={(e) => changeStage(d._id, e.target.value)}
              className="select-field w-full text-sm mb-3"
            >
              {stages.map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
            </select>
            <div className="flex gap-2">
              <button onClick={() => sendQuote(d._id)} className="chip chip-orange flex-1 justify-center text-sm">
                عرض سعر
              </button>
              <span className="text-xs text-[var(--muted)] self-center">متابعات: {d.followUpCount || 0}</span>
            </div>
          </div>
        ))}
        {!deals.length && (
